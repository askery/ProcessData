...
logFile = "/media/data/rowData_201304_201601/NEG_20151125.TXT"  # FILE FROM SYSTEM
#
...
def makeColumns(line):
    ...
    sec     = time[2].split('.')        # separate seconds from miliseconds
    key     = symb + hour + minu + sec[0] + sec[1]  # key definition as concatenation of columns
    # OUTOUT FORMAT WITHOUT 'U', ',' OR '()'
    output  = date + "   " + str(hour)+":"+str(minu)+":"+str(sec[0])+"."+str(sec[1]) + "   " +  str("%.2f" % float(pieces[3])) + "   " + str(int(pieces[4])) 
    return (key, output)

#5 function to separate distinct stocks in different output files
def separate(a):
    if stock in a[0]:
        return (a[0],a[1])    
...
result  = logData.filter(filterStock).map(makeColumns).sortByKey()
